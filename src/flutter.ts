...
(<any>window).stopClient = async (i: number) => {
  let client = Client.from_index(i);
  await client?.stop()
}
...
(<any>window).daoBalance = async (client_index: number, dao_id: number, address: string) => {
  let client = Client.from_index(client_index);
  if (client.api == null) {
    throw "client is not start"
  }
  let balance = new Asset(client);
  let data = await balance.balance(dao_id, address);
  return JSON.stringify(data);
}
